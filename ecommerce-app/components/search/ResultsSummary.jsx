    ...
    <div className="sm:flex sm:flex-1 sm:items-center sm:justify-between my-2">
      ...
      <div className="flex justify-between">
        <div className="sm:hidden block flex-1">
          <button
            onClick={openModal}
            className="bg-black flex mx-auto justify-center items-center rounded-full h-10 w-10 text-white"
          >
            <FiltersIcon />
          </button>
        ...
        <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
          <button
            className={clsx(pageNumberStyle.default, defaultPageNumberStyle)}
            onClick={() => onPageUpdate(currPage - 1)}
            disabled={currPage == 1}
          >
            <span className="sr-only">Previous</span>
            <ChevronLeft width={24} />
          </button>
          <Pagination pagesTotal={totalPages} onClickPage={onPageUpdate} currentPage={currPage} isExpanded={false} />
          <button
            className={clsx(pageNumberStyle.default, defaultPageNumberStyle)}
            onClick={() => onPageUpdate(currPage + 1)}
